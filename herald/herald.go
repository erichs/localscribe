...
// Heartbeat format (now *after* a leading timestamp): 
//   2024/12/31 17:22:45 %%% heartbeat 2024-12-31T17:22:45Z
// Normal transcript lines: anything else (no special prefix after we strip).
...
// Each time we find a line starting with "###" (after stripping any leading
// timestamp), we parse it as a command and spawn a goroutine to handle it.
func watchLoop(ctx context.Context, cfg Config) error {
    ...
    for {
        ...
        default:
            ...
            // Strip leading "YYYY/MM/DD HH:MM:SS " if present
            stripped := stripLeadingTimestamp(line)

            // Check if (after stripping) it starts with "###"
            if strings.HasPrefix(stripped, "###") {
                cmdStr := strings.TrimPrefix(stripped, "###")
            ...
            // Otherwise, ignore normal transcript or metadata lines
        }
    }
}

// stripLeadingTimestamp checks if a line begins with something like:
//   2024/12/31 17:22:45 
// If so, it strips that prefix (including the trailing space).
func stripLeadingTimestamp(line string) string {
    // A naive approach: check for a 19-char datetime + space, e.g. "2024/12/31 17:22:45 "
    // 2024/12/31 (10 chars) + space (1) + 17:22:45 (8 chars) = 19 plus 1 space = 20 total
    // We'll do a quick length & format check, then strip if it matches.
    if len(line) < 20 {
        return line
    }
    potentialTS := line[:19]
    // We won't do a full parse, just a quick sanity check
    if isLikelyTimestamp(potentialTS) && line[19] == ' ' {
        // strip the first 20 characters
        return line[20:]
    }
    return line
}

// isLikelyTimestamp checks a naive pattern: YYYY/MM/DD HH:MM:SS
func isLikelyTimestamp(s string) bool {
    if len(s) != 19 {
        return false
    }
    // Very basic checks
    // e.g. 2024/12/31 17:22:45
...
// Because transcript lines no longer strictly *start* with "%%% heartbeat ",
// we do a substring check for "%%% heartbeat ". Then, if found, we parse the
// *right-hand side* as RFC3339 to see if it's older than cutoff.
//
...
outerLoop:
    for offset > 0 {
        ...
        for i := len(lines) - 1; i >= 0; i-- {
            ...
            // If we see "%%% heartbeat " anywhere, attempt to parse the trailing timestamp
            if idx := bytes.Index(l, []byte("%%% heartbeat ")); idx != -1 {
                // e.g. "2024/12/31 17:22:45 %%% heartbeat 2024-12-31T17:22:45Z"
                // We'll parse everything after that substring as the RFC3339 time.
                heartbeatSuffix := l[idx+len("%%% heartbeat "):]
                // Attempt to parse as RFC3339
                parsed, errTime := time.Parse(time.RFC3339, string(heartbeatSuffix))
                if errTime != nil {
                    // If parse fails, keep searching
                    continue
                }
                // if it's older than cutoff, we stop here
...
// filterUnwanted removes lines that start with "%%%" or "###" (after
// we strip any leading timestamp).
func filterUnwanted(lines []string) []string {
    var out []string
    for _, ln := range lines {
        stripped := stripLeadingTimestamp(ln)
        if strings.HasPrefix(stripped, "%%%") {
            continue
        }
        if strings.HasPrefix(stripped, "###") {
